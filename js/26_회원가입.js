...
inputId.addEventListener("input", e => {
    ...
    if(value.trim().length == 0) {
        ...
        // 유효성 검사 여부 체크 객체에서 input를 false로 변경
...
inputPw.addEventListener("input", e => {
    ...
    // ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** 
    // 함수 이름만 작성하면 함수 이름을 작성한 부분에 함수 정의 코드가 그대로 대입된다!! *
    // ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** 
...
inputName.addEventListener("input", e => {
    ...
    // 유효한 경우
    if(regExp.test(value)){ 
        span.textContent = "유효한 이름 형식입니다";
        span.classList.add("check");
        span.classList.remove("error");
        checkList["inputName"] = true;
    } else{
        span.textContent = "유효하지 않은 이름 형식입니다";
        span.classList.add("error");
        span.classList.remove("check");
        checkList["inputName"] = false;
    }
...
signupForm.addEventListener("submit", e => {
    ...
    // 아이디,비밀번호,비밀번호 확인,이름이 모두 유효한지 확인
    ...
    // 체크된 name='gender'  라디오 버튼 얻어오기 -> 체크된 라디오 버튼이 없다면 null
