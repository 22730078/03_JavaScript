...
document.querySelector("#btn2a").addEventListener("click",function(){
    ...
    /* 참조란?
        - 객체 지향 언어에서 배열,객체 등 object는 값을 여러 개를 가지고 있는 자료형인데 
          변수는 값을 1개만 저장할 수 있기 때문에 object를 메모리 다른 곳에 저장하고
          저장된 곳의 주소를 변수에 저장해서 필요할 때마다 해당 주소를 찾아가 
    ...
    arrayTest(arr2);
     ...
     /* 왜 멜론이 아니라 딸기가 출력? 
        -> arrayTest(arr2) 호출 시 arr2 배열을 통째로 복사해서 전달하는 것이 아닌 
...
// 전달받은 수(num)을 x제곱하여 반환
function pow (num,x) {

    let result = 1; //곱했을 떄 결과에 영향을 주지 않기 위헤 1 초기화
