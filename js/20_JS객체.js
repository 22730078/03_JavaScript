...
btn1.addEventListener("click", () => {
    ...
    // 3. 객체에 K:V 추가
    // 원하는 key를 생각해두고 객체명.key = value; 또는 객체명["key"] = value;
    ...
    // 4. 특정 key에 value 수정하기 -> 덮어쓰기
    product.price = 1800000;
    console.log(product);

    // 5. 원하는 K:V 삭제하기 (delete 연산자 사용) -> delete 객체명.key
...
btn2.addEventListener("click", () => {
    ...
    smartPhone.information = function() {
        ...
        /* this 참조 변수 
        - 작성되는 위치에 따라서 의미가 다름
        (ex) <button onclick = testFn(this)> --> 이 때 this  == 클릭된 요소 (버튼) 
        (ex) 객체명.메서드명 = function(){
            this.변수명 = "값"; --> 이 때 this  == 메서드가 작성된 객체
        }*/ 
...
function Student(name,grade,ban,number,score){
    ...
    // 속성
    // (전달받은 값(parameter)을 현재 객체(this)에 속성으로 추가)
    this.name   = name;   
    this.grade  = grade; 
    this.ban    = ban; 
    this.number = number; 
    ...
    this.introduce = function(){
        console.log(`${this.grade}학년 ${this.ban}반 ${this.number}번 ${this.name} ${score}점 입니다.`);
    }
}
...
btn3.addEventListener("click", ()=>{
    ...
    const student2 = new Student("김길동",2,7,14,100);
    ...
    // 생성자 함수 Student를 이용해서 객체 생성 후 바로 StudentList에 추가
    StudentList.push(new Student("김미영",5,8,4,90));
