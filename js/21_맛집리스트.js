...
/* 문자열.split("구분자")  -> 문자열을 "구분자"로 쪼개서 배열로 반환 */

/* 전역변수로 식당 목록을 저장할 배열을 선언 */
const resList = [];

// 샘플 식당 데이터 추가
resList.push(new Restaurant("김밥천국", "김밥,라면", "서울시 중구 어딘가"));
resList.push(new Restaurant("왕가탕후루", "샤인머스켓, 딸기, 통귤", "서울시 서대문구 어딘가"));
...
// ex) 입력된 값이 없을 경우 비밀번호 형식 맞지 않을 경우 입력된 값의 범위가 옳지 않을 경우
    ...
    // 조회 버튼 클릭 시 #printList의 이전 내용을 모두 삭제하고 resList의 요소를 이용해 li요소를 만들어 추가
    ...
    selectBtn.addEventListener("click", () => {
        ...
        // 만들어진 li의 내용으로 resList[i]의 information() 메서드 반환 값 대입
