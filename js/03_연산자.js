...
//Element : HTML 요소
//ById : 아이디가 일치하는
// -> HTML 문서 내에서 ()안에 아이디가 일치하는 요소를 얻어오겠다
...
function plusFn(){
    ...
    // -> input 요소에 작성된 값은 무조건 문자열(string) 형태라서 더했을 때 이어쓰기 되는 문제가 발생한다!!!
    // [해결방법] 문자열(string)을 숫자(number)로 변경하는 코드를 수행 
    // --> 데이터 파싱(data parsing) : 데이터의 종류를 바꿈

    /* 숫자만 작성된 문자열("123")을 진짜 숫자(123)로 바꾸는 방법!!! -> Number("123") -->123 */
...
function check1(){
    ...
    // 컴퓨터한테 연산은 +-*/ 뿐만 아니라 코드 하나하나 실행하는 것들이 연산이다!!

    //전위 연산(++count, --count) 확인 -> 다른 연산보다 먼저 수행 / -> count 값이 먼저 증가, 감소된 후 console에 출력이 된다
    ...
    //후위 연산(count++, count--) 확인 -> 다른 연산이 다 끝나고 마지막에 수행
    count = 50;
    console.log("-------------------");
