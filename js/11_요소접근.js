...
function classTest(){
    ...
    //cls-test 요소 모두 얻어오기 ->HTML Collection(유사배열)
    //유사배열 : 배열처럼 index, length를 가지고 있으나 배열 전용 기능(메서드)를 제공하지 않음
...
function classTest2(){
    ...
    //클래스가 cls-test2인 요소를 모두 얻어오겠다 ->HTML Collection(유사배열) 형태로 얻어옴
    const inputList = document.getElementsByClassName("cls-test2");

    /* (!!중요!!) 
        요소들을 한번에 얻어오게 되면 배열 형태로 반환된다. 
        -> 요소를 다루고 싶으면 index를 이용해 배열 요소를 하나씩 꺼내서 다뤄야 한다!!  */
...
function tagTest(){
    ...
    //HTML문서에 존재하는 모든 li 태그 요소를 얻어와 배열로 묶어서 반환
...
function nameTest(){
    ...
    let count="";
     ...
     for(let i=0; i<hobbyList.length; i++){
        ...
        // input 요소.checked         -> 요소가 체크되어 있으면 true, 아님 false
        // input 요소.checked = true  ->  해당 요소를 체크
        // input 요소.checked = false ->  해당 요소를 체크 해제

        //체크박스의 값, 체크여부 출력
        console.log(hobbyList[i].value, hobbyList[i].checked);

        //체크 여부를 검사해서 체크가 되어 있다면 체크된 요소의 값을 str 변수에 누적 + count 변수 값을 1 증가
     ...
     document.getElementById("name-div").innerHTML = `${str}<br><br>선택된 취미 갯수 : ${count}`;
}
...
function cssTest(){
    ...
    /* 
    [요소 1개]
      1) document.querySelector("CSS 선택자"); -> 선택자가 선택한 요소 중 첫 번째 요소를 반환
    [요소 n개 -> 배열]
      2) document.querySelectorAll("CSS 선택자"); -> 선택한 요소 모두를 배열(NodeList, 유사배열) 형태로 반환
    */
    ...
    //첫번째 자식 div 선택  (방법2) -> querySelector()의 첫번째 요소만 선택한다는 특징 활용
...
function readValue(){
    ...
    if(input.value.trim().length == 0){
        ...
        // 현재 수행중인 함수를 종료 + 호출한 곳으로 돌아감 (2개의 의미를 가짐)
        return;
    }
